...
5. **Pipeline de dados** (`lyrics`, `enrichment`, `_async_http`, `scripts/`, todos em `src/music_search/`) — gera os parquets versionados em `data/derived/final/`. Não roda no caminho crítico de uma busca; só quando se quer regenerar dados.
...
```
HTTP /search?q=… → web/app classifica intent por heurística
...
- **Sem LLM no caminho crítico**: a classificação de intent e a materialização de entidades são 100% determinísticas (heurística em `web/app.py` e parsing local em `enrichment/pipeline.py`). Não reintroduzir clientes LLM no caminho de uma query — qualquer chamada externa síncrona vira gargalo grave no VPS.
